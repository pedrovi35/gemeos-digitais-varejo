...
Fronteira entre UI e domínio:

| Serviço | Função |
|---------|--------|
| `risk_service.py` | API unificada de scores, rankings, matriz composta e explicações |
| `backtest_service.py` | Score walk-forward out-of-time num corte de tempo (`score_window`) |
| `counterfactual_service.py` | Simulador contrafactual baseline × gêmeo (prova de valor) |
