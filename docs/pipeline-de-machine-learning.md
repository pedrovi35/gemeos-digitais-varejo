...
## Backtest e prova de valor

Além do treino/scoring de produção, o pipeline suporta um modo de avaliação
out-of-time: `score_window` (em `services/backtest_service.py`) treina um modelo
em memória num corte passado (`Trainer.fit(df, persist=False)`), scora as
entidades no corte e cruza com os eventos reais da janela seguinte. O
`counterfactual_service` mede o valor da intervenção. Detalhes em
...
# Backtest walk-forward out-of-time
