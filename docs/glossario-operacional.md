...
| **Backtest walk-forward** | Avaliação out-of-time: treina num corte `T`, scora em `T`, valida com eventos de `(T, T+H]`. |
| **AUC out-of-time** | ROC-AUC medida contra labels da janela futura nunca vista no treino. |
| **Precision@k** | Fração dos top-k de maior risco que realmente sofreram a ruptura. |
| **Calibração** | Aderência entre risco predito e taxa real (reliability diagram). |
| **Lead time do alerta** | Dias entre o alerta do modelo e a ruptura efetiva. |
| **Target-leak** | Reuso de uma feature do período corrente no target — infla a AUC in-sample. |
| **Contrafactual** | Comparação baseline (gêmeo não age) × gêmeo age, para medir valor. |
