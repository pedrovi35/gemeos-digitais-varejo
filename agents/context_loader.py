...
_TEMPLATES: dict[str, str] = {
    ...
    """,
    "sku_meta": """
        SELECT s.*, sup.name AS supplier_name, sup.tier,
               sup.otd_rate, sup.lead_time_days
        FROM dim_sku s
...
def run_template(
    ...
    *,
    ...
    """Run a whitelisted SQL template.

    `filters` supplies values for `{placeholder}` slots in the template SQL
    (e.g., `store_filter`). Unsupplied placeholders are rendered as empty
    strings so optional predicates simply drop out of the WHERE clause.
    """
    if name not in _TEMPLATES:
        raise KeyError(f"template not whitelisted: {name}")
    sql = _TEMPLATES[name]
    if "{" in sql:
        # Default every placeholder to '' so missing keys don't KeyError
        import string
        keys = {f for _, f, _, _ in string.Formatter().parse(sql) if f}
        rendered = sql.format(**{k: "" for k in keys} | (filters or {}))
    else:
        rendered = sql
    try:
        conn = get_connection()
        return conn.execute(rendered, params or []).fetch_df()
    except Exception as exc:
        logger.exception(f"context_loader · template '{name}' failed: {exc}")
        raise


def _store_filter(store_id: str | None, alias: str | None = None) -> tuple[str, list[Any]]:
    """Build an optional `AND [alias.]store_id = ?` predicate."""
    if store_id:
        col = f"{alias}.store_id" if alias else "store_id"
        return f"AND {col} = ?", [store_id]
    return "", []


def _category_filter(category: str | None, alias: str = "k") -> tuple[str, list[Any]]:
    if category:
        return f"AND {alias}.category = ?", [category]
    return "", []
...
def load_general_context(store_id: str | None = None) -> dict:
    ...
    kpi    = run_template("kpi_today",     sp, filters={"store_filter": sf})
    trend  = run_template("kpi_trend_14d", sp, filters={"store_filter": sf})
...
def load_rupture_context(store_id: str | None = None,
    ...
    sf, sp = _store_filter(store_id, alias="i")
    cf, cp = _category_filter(category, alias="k")
    ruptured = run_template(
        "ruptured_skus", sp + cp,
        filters={"store_filter": sf, "category_filter": cf},
    )
    risks    = run_template("top_risk_categories")
...
def load_sku_context(sku_id: str, store_id: str) -> dict:
    ...
    sku_meta  = run_template("sku_meta",         [sku_id])
    inventory = run_template("inventory_latest", [sku_id, store_id])
    return {
