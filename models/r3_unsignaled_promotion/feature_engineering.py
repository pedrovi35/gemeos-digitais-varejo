...
class FeatureEngineering(BaseFeatureEngineering):
    ...
    def build(self, *, horizon_days: int = 7,
        ...
        # Pure-future target: was there an unsignaled-promo event in the next H days?
        # An event = registered-flag==0 AND uplift>0.5. Mixing current uplift into the
        # target (as in the old rule) leaked the feature back into the label.
        sales["_event"] = (
            (sales["promotion_registered"] == 0) & (sales["uplift_variation"] > 0.5)
        ).astype(int)
        fut_event = (
            sales.groupby(["store_id", "sku_id"])["_event"]
                 .transform(lambda s: s.shift(-1).rolling(horizon_days, min_periods=1).max())
        )
        sales[self.target_col] = fut_event.fillna(0).astype(int)
        sales = sales.drop(columns=["_event"])
        sales[self.entity_key] = sales["store_id"].astype(str) + "·" + sales["sku_id"].astype(str)
        sales["d"] = pd.to_datetime(sales["d"])
        out = sales[list(FEATURE_COLS) + [self.target_col, self.entity_key, "d", "store_id", "sku_id"]]
