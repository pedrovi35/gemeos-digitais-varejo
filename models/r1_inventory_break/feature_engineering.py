...
class FeatureEngineering(BaseFeatureEngineering):
    ...
    def build(self, *, horizon_days: int = 7,
        ...
        # Target = inventory-break signal observable in the current or next horizon window.
        # The seed snapshots are sparse (~7 days per store-sku), so a hard -7d shift
        # erases all future values; we mix current-period signals with a short look-ahead.
        fut_so = df.groupby(["store_id", "sku_id"])["stockout_frequency"].shift(-1).fillna(0)
        acc_threshold = df["stock_accuracy"].quantile(0.20)
        df[self.target_col] = (
            (df["stockout_frequency"] >= 0.10) |
            (fut_so >= 0.10) |
            (df["stock_accuracy"] <= acc_threshold)
        ).astype(int)
