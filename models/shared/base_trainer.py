...
class BaseTrainer:
    ...
    # ── Public ──────────────────────────────────────────────────────
    def fit(self, df: pd.DataFrame, *, time_col: str | None = "d",
        ...
        self.last_xgb_, self.last_lgbm_ = xgb_model, lgbm_model
        return metrics

    def predict(self, X: pd.DataFrame) -> np.ndarray:
        """Predict using the in-memory ensemble from the last fit() call."""
        return self._ensemble_predict(self.last_xgb_, getattr(self, "last_lgbm_", None), X)
    ...
    def _temporal_split(self, df: pd.DataFrame, X: pd.DataFrame, y: np.ndarray,
        ...
        # Tiny datasets (e.g., supplier-level R5 with ~7 rows) can't sustain a temporal
        # split — fall back to stratified random so both folds have positives.
        if len(X) < 40 or not (time_col and time_col in df.columns):
            stratify = y if (y.sum() > 1 and (len(y) - y.sum()) > 1) else None
            return train_test_split(X, y, test_size=self.cfg.test_size,
                                    random_state=self.cfg.random_state, stratify=stratify)
    ...
    def _fit_xgb(self, X_tr, y_tr, X_va, y_va):
        ...
        eval_ok = len(np.unique(y_tr)) > 1 and len(np.unique(y_va)) > 1
        if eval_ok:
    ...
    def _fit_lgbm(self, X_tr, y_tr, X_va, y_va):
        ...
        # LightGBM raises if y_va carries a label unseen in y_tr — guard small folds.
        eval_ok = len(np.unique(y_tr)) > 1 and set(np.unique(y_va)).issubset(set(np.unique(y_tr)))
        if eval_ok:
