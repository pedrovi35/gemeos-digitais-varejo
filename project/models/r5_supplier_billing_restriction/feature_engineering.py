...
class FeatureEngineering(BaseFeatureEngineering):
    ...
    def build(self, *, horizon_days: int = 7) -> pd.DataFrame:
        ...
        df = sup.merge(inv, on="supplier_id", how="left").merge(orders, on="supplier_id", how="left")
        ...
        df["d"] = pd.Timestamp.utcnow().normalize()
        return df[list(FEATURE_COLS) + [self.target_col, self.entity_key, "d", "supplier_id"]]
