...
st.set_page_config(
    page_title="System Console · Gêmeo Digital",
...
c2.metric("Boot", f"{metrics.get('boot_ms', 0):.0f} ms")
c3.metric("Queries", metrics["query_count"])
c4.metric("Errors", metrics["error_count"])

section_banner("HEALTH CHECKS")
for k, v in health.checks.items():
    st.write(f"{'✅' if v else '❌'} **{k}** — {health.details.get(k, '—')}")
